...
def generate_metadata():
    ...
    for word in Word.objects.all():
        ...
        record.dc_contributor = ';'.join(['tima:citizen_scientists:%s' % user for user in AssociationHistory.objects.filter(association__word=word).distinct().values_list('user__username', flat=True) if user])
        ...
        record.dc_relation = ';'.join(['tima:word:%s' % word.id for word in word.word.all()] + ['tima:word:%s' % word.id for word in word.association.all()] + ['tima:citizen_scientists:%s' % user for user in AssociationHistory.objects.filter(association__word=word).distinct().values_list('user__username', flat=True) if user])
        record.dc_coverage = None
        record.dc_rights = 'http://creativecommons.org/licenses/by/4.0/'
        record.save()

    for header in Header.objects.filter(identifier__startswith='tima:word').exclude(identifier__in=Word.objects.annotate(identifier=Concat(V('tima:word:'), 'id')).values_list('identifier', flat=True)):
